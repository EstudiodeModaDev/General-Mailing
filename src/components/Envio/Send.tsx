...
function renderTemplate(template: string, row: Record<string, any>, columnMapping: Record<string, string>) {
...
export const Step3Send: React.FC <Step3Props>= ({excelData, recipientColum, subject, htmlTemplate, columMapping, onDone, setLog}) => {
    ...
    const validation = React.useMemo(() => {
        ...
        // 3. Correos válidos
        const validEmails = excelData.filter(r =>
            isValidEmail(r[recipientColum])
        );
        ...
        // 6. Variables usadas vs mapeadas
        const usedVars = [
            ...extractVars(subject),
            ...extractVars(htmlTemplate),
        ];
    ...
    try {
        ...
        const built = buildMailPayloadForRow({
            row,
            recipientColumn: recipientColum,
            columnMapping: columMapping,
            subjectTemplate: subject,
            htmlTemplate,
        });

        if (!built) {
            // ✅ cuenta como “intentado” si quieres que el progreso avance igual
            resultados.push({
            asunto: subject,
            correo: String(row[recipientColum] ?? ""),
            estado: "FAILED",
            fecha: new Date(),
            });
            setSend((prev) => prev + 1);
        ...
        try {
            ...
            resultados.push({
            asunto: built.payload.message.subject,
            correo: built.recipient,
            estado: "SENT",
            fecha: new Date(),
            });
        } catch {
            ...
            // ✅ sube SIEMPRE, haya éxito o fallo
            setSend((prev) => prev + 1);
        }
        }
        ...
        <section id="view-step-3" className="step-view grid grid-cols-1 lg:grid-cols-12 gap-8 fade-in h-full">
                ...
                <div className="p-6 bg-indigo-50 rounded-2xl border border-indigo-100 flex gap-4">
                    <div className="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center flex-shrink-0 text-indigo-600">
                        ...
                        <p className="text-xs text-indigo-900 leading-relaxed font-medium">
                            Los correos se enviarán de forma estrictamente individual. Los destinatarios no podrán verse entre sí.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="lg:col-span-7 flex flex-col">
                ...
                <div className="relative z-10 w-full max-w-sm">
                    <span className="text-indigo-200 text-sm font-bold uppercase tracking-widest mb-6 block">
                        Volumen de Entrega
                    </span>
                ...
                {submitting ?
                    <div className="mb-10" id="progress-bar-container">
                        <div className="flex justify-between text-white text-[10px] font-black uppercase mb-2">
                            <span id="send-status-text">Iniciando... (Enviados: {send} de {cantidad})</span>
                            <span id="pct-display">{(send/cantidad)*100}%</span>
                        </div>

                        <div className="w-full bg-white/20 h-4 rounded-full overflow-hidden border border-white/10 p-1">
                            <div id="send-progress" className="h-full bg-emerald-400 rounded-full transition-all duration-300 shadow-[0_0_10px_rgba(52,211,153,0.6)]" style={{ width: `${(send/cantidad)*100}%` }}/>
                        </div>
                    </div> : null
                }

                <button id="start-send-btn" onClick={(e) => handleSubmit(e)} disabled={submitting} className={`w-full py-5 rounded-2xl text-xl font-black transition flex items-center justify-center gap-3 bg-white text-indigo-700 hover:bg-slate-100 shadow-2xl shadow-indigo-900/40 active:scale-95`}>
